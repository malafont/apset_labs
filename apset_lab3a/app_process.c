...
static const char *ecc_weierstrass_prime_key_string[] = {
  ...
  "ECC P384",
  "ECC P521",
  "ECC Custom secp256k1",
  "ECC Custom brainpoolP224r1",
  "ECC Custom brainpoolP512r1"
#endif
};
...
void app_process_action(void)
{
  ...
  switch (app_state) {
    ...
    case SELECT_WEIERSTRASS_KEY:
...
#if (_SILICON_LABS_SECURITY_FEATURE == _SILICON_LABS_SECURITY_FEATURE_VAULT)
        if (ecc_weierstrass_prime_key_select == 7) {
#else
      ...
      if (enter_press) {
        enter_press = false;
        if (ecc_weierstrass_prime_key_select == 4)
            set_active_domain(DOMAIN_SECP256K1);
        else if (ecc_weierstrass_prime_key_select ==5)
            set_active_domain(DOMAIN_BRAINPOOLP224R1);
        else if (ecc_weierstrass_prime_key_select == 6)
...
static void print_key_algo_option(void)
{
  if (asymmetric_key_algo_select == 0) {
    printf("\n  . Current ECC Weierstrass Prime key is %s.\n",
           ecc_weierstrass_prime_key_string[ecc_weierstrass_prime_key_select]);
    printf("  + Press SPACE to select ECC Weierstrass Prime key (%s/%s/%s/%s/%s/%s/%s),"
           ...
           ecc_weierstrass_prime_key_string[5],
           ecc_weierstrass_prime_key_string[6]);
    app_state = SELECT_WEIERSTRASS_KEY;
  } else {
